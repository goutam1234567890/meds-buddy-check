...
import { getMedications, getAdherence } from "@/api/medications";
...
const CaretakerDashboard = () => {
  ...
  // --- Patient name logic (set as needed) ---
  ...
  async function fetchLiveData() {
    ...
    try {
      ...
      setAdherence(adh.adherence ?? null);
    } catch (err: any) {
  ...
  // Utility - combine all taken_dates
  const allTakenDates = medications.flatMap((med) => med.taken_dates || []);
  const takenDatesSet = new Set(allTakenDates);
  ...
  const recentActivity = Array.from({ length: 7 }).map((_, i) => {
    ...
    const taken = medications.every((med) =>
      Array.isArray(med.taken_dates) ? med.taken_dates.includes(dateStr) : false
    );
    return {
      date: dateStr,
      taken,
      time: taken ? "08:00 AM" : null, // Could be improved with real log time if you track it
      hasPhoto: false, // Update if you add photo logic
    };
  });

  // Current streak (consecutive days all meds taken)
  let currentStreak = 0;
  for (let i = 0; i < 30; i++) {
    const date = subDays(today, i);
    const dateStr = format(date, "yyyy-MM-dd");
    ...
    if (allMedsTaken) currentStreak++;
    else break;
  }

  // Missed doses: count days in last 30 days with at least one med not taken
  const last30Days = Array.from({ length: 30 }).map((_, i) =>
    format(subDays(today, i), "yyyy-MM-dd")
  );
  const missedDoses = last30Days.filter(
    (date) =>
      medications.length > 0 &&
      medications.some(
        (med) =>
          Array.isArray(med.taken_dates) && !med.taken_dates.includes(date)
      )
  ).length;
  const takenThisMonth = last30Days.filter((date) =>
    medications.length > 0 &&
    medications.every(
      (med) => Array.isArray(med.taken_dates) && med.taken_dates.includes(date)
    )
  ).length;
  const remainingThisMonth = 30 - takenThisMonth - missedDoses;

  // Daily: completed if all meds taken today
  const allMedsTakenToday =
    medications.length > 0 &&
    medications.every((med) =>
      Array.isArray(med.taken_dates)
        ? med.taken_dates.includes(format(today, "yyyy-MM-dd"))
        : false
    );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-green-500 to-blue-500 rounded-2xl p-8 text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white/10 rounded-xl p-4 backdrop-blur-sm">
            <div className="text-2xl font-bold">{takenThisMonth}</div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center justify-between p-3 bg-accent/50 rounded-lg">
                  <div>
                    <h4 className="font-medium">All Medications</h4>
                  ...
                  <Badge
                    variant={
                      allMedsTakenToday ? "secondary" : "destructive"
                    }
                  >
                    {allMedsTakenToday ? "Completed" : "Pending"}
                  </Badge>
                </div>
              </CardContent>
            </Card>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <Progress value={adherence || 0} className="h-3" />
              ...
              <div className="grid lg:grid-cols-2 gap-6">
                <div>
                  <Calendar
                    ...
                    components={{
                      DayContent: ({ date }) => {
                        const dateStr = format(date, "yyyy-MM-dd");
                        const isTaken = medications.every((med) =>
                          Array.isArray(med.taken_dates)
                            ? med.taken_dates.includes(dateStr)
                            : false
                        );
                ...
                <div>
                  ...
                  <div className="space-y-4">
                    {medications.every((med) =>
                      Array.isArray(med.taken_dates)
                        ? med.taken_dates.includes(format(selectedDate, "yyyy-MM-dd"))
                        : false
                    ) ? (
      ...
      {error && (
        <div className="text-red-700 mt-2 font-medium">{error}</div>
      )}
    </div>
  );
};
