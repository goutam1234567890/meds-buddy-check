...
import { uploadProofPhoto } from "@/api/medications";
...
interface MedicationTrackerProps {
  ...
  isToday: boolean;
  medicationId?: number; // Add medicationId prop
  proofPhotoUrl?: string; // Add proofPhotoUrl prop
}

const MedicationTracker = ({ date, isTaken, onMarkTaken, isToday, medicationId, proofPhotoUrl }: MedicationTrackerProps) => {
  ...
  const handleMarkTaken = async () => {
    if (selectedImage && medicationId) {
      setUploading(true);
      try {
        await uploadProofPhoto(medicationId, selectedImage);
      } catch (e) {
      ...
      <div className="space-y-4">
        ...
        {proofPhotoUrl && (
          <div className="flex justify-center mb-4">
            <img src={proofPhotoUrl} alt="Proof" className="max-h-40 rounded-lg border" />
          </div>
        )}
  ...
  return (
    <div className="space-y-6">
      ...
      <Button
        ...
        disabled={!isToday || uploading}
      >
        <Check className="w-5 h-5 mr-2" />
        {uploading ? "Uploading..." : isToday ? "Mark as Taken" : "Cannot mark future dates"}
      </Button>
